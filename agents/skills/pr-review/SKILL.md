...
1. **Gather context**: Fetch the PR details (title, description, changed files, diffs). If no PR is specified, use the active/current PR.
...
7. **Suggested improvements** — concrete, not generic (no "add comments" or "add docstrings")
8. **Highlight** — one specific positive aspect of the changes (not tests)

## Constraints
- Always map feedback to specific file paths
- Never give generic feedback ("add comments", "add docstrings", "improve naming")
- Every feedback point must be actionable with a clear location
